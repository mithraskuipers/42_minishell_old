...
t_exec_element	*new_exec_element(void)
{
	...
	if (!new)
	{
		msg_ret_int("new_exec_element()", FAILURE);
		return (NULL);
	}
	new->type = -1;
	new->next = NULL;
...
t_exec_element *assign_exec_element(t_exec_element *element, int type, t_env **env, char **input)
{
	...
	if (element->type == tkn_cmd)
	{
		...
		if (!element->value)
		{
			msg_ret_int("assign_exec_element()", FAILURE);
			return (NULL);
		}
	}
...
t_exec_element	*exec_list_generator(t_exec_element *line_blks, int n_blocks, t_env **env)
{
	...
	if (!head)
	{
		msg_ret_int("token_list_generator()", FAILURE);
		return (NULL);
	}
	curr_element = head;
	while (iter < n_blocks)
	{
		...
		{
			msg_ret_int("exec_list_generator()", FAILURE);
